...
/**
 * Adopted from
 * http://ikayzo.org/svn/beanshell/BeanShell/engine/src/bsh/engine/
 * <p/>
 * Notes This engine supports open-ended pluggable scriptcontexts
 */
public class BshScriptEngine
     ...
     * difference here in BeanShell because BeanShell will do a few extra things in
     * the string case... e.g. tack on a trailing ";" semicolon if necessary.
     */
    private Object evalSource( Object source, ScriptContext scriptContext )
        ...
        try {
            ...
            // set it as the cause ?
            ScriptException se = new ScriptException( e.toString(), e.getErrorSourceFile(), e
                    .getErrorLineNumber() );
    ...
    private static NameSpace getEngineNameSpace( ScriptContext scriptContext ) {
        NameSpace ns = (NameSpace)scriptContext.getAttribute( engineNameSpaceKey,
     ...
     * @return An subclass of {@code CompiledScript} to be executed later using one
     *         of the {@code eval} methods of {@code CompiledScript}.
    ...
    /**
     * Compiles the script (source read from {@code Reader}) for later execution.
     * Functionality is identical to {@code compile(String)} other than the way in
     * which the source is passed.
     * 
     * @param script The reader from which the script source is obtained.
     * @return An implementation of {@code CompiledScript} to be executed later using
     *         one of its {@code eval} methods of {@code CompiledScript}.
     * @throws ScriptException if compilation fails.
     ...
     * retained in the state of the {@link ScriptEngine}.
     *
     ...
     * @throws javax.script.ScriptException if an error occurrs during invocation of
     *         the method.
     * @throws NoSuchMethodException if method with given name or matching argument
     *         types cannot be found.
     * @throws NullPointerException if method name is null.
     ...
     * interpreter. The methods of the interface may be implemented using the {@code
     * invoke} method.
     * 
     ...
     *         is unavailable, i. e. if compiled methods in the {@code ScriptEngine}
     *         cannot be found matching the ones in the requested interface.
     * @throws IllegalArgumentException if the specified {@code Class} object does
...
//        }
     ...
     * scripting object compiled in the interpreter. The methods of the interface may
     * be implemented using invoke(Object, String, Object...) method.
     * 
     * @param thiz The scripting object whose member functions are used to implement
     ...
     *         cannot be found matching the ones in the requested interface.
     * @throws IllegalArgumentException if the specified {@code Class} object does
     *         not exist or is not an interface, or if the specified Object is null
